...
export const meta = () => ([{title: "Resumind | Review"}, {
...
const resume = () => {
    ...
    useEffect(() => {
        const loadResume = async () => {
            ...
            if (data.rephrasedCoverLetter) setRephrasedCoverLetter(data.rephrasedCoverLetter);
            ...
            <section
                ...
                {imageUrl && resumeUrl && (<div
                    ...
                    <a href={resumeUrl} target="_blank" rel="noreferrer" className="w-full h-full">
                        <img src={imageUrl} alt="image" className="w-full h-full object-cover object-center"
                             title="resume"/>
                    </a>
                </div>)}
            </section>
                    ...
                    <div className="flex flex-col gap-8 animate-in fade-in duration-1000">
                        ...
                        {rephrasedCoverLetter && (
                            <div className="bg-white p-4 rounded-md shadow-sm">
                                <h3 className="text-lg font-semibold mb-2">Rephrased Cover Letter</h3>
                                <pre className="whitespace-pre-wrap text-sm">{rephrasedCoverLetter}</pre>
                            </div>
                        )}
                    </div>
                </div>) : (<img src="/images/resume-scan-2.gif" alt="resume scan" className="w-full"/>)}
            </section>
        </div>
    </main>);
};
